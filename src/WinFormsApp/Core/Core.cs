...
namespace MpSoft.SmtpFiddler.Core
{
	...
	public static class FormatMethods
	{
		...
		{
			return new StringBuilder(Encoding.ASCII.GetString(bytes)).Replace((char)0, ' ').ToString();
		}
		...
		{
			return size.ToString("N0") + " B [" + (size / 1024).ToString("N0") + " KB]";
		}
	...
	{
		 public static T GetResource<T>(Type type,string resName) where T:class
		 {
			  try
			  {
					return new System.ComponentModel.ComponentResourceManager(typeof(FormMain))
							  .GetObject(resName) as T;
			  }
			  catch
			  {
					return null;
